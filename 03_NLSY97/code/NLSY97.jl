...
# ╔═╡ 2c42306d-2da0-47eb-b3ec-38c524bd4059
using DataFrames, CSV, Dates, Statistics, ReadStat, ShiftedArrays, Plots, StatFiles


# ╔═╡ a9caca16-3d36-4314-b310-237ef79effee
df = CSV.read("D:\PSID_SHELF\03_NLSY97\NLSY97.csv")
println(first(df, 5))


# ╔═╡ 27465ccc-1745-4021-8ed2-127daf9ccc03
rename!(df, Symbol("_XRND$i") => Symbol("EMPSTATUS_$i") for i in 1:828)


# ╔═╡ db5d0e1a-b395-4575-aabf-7257ef3767fe
begin
	function rename_EMP_STATUS!(df)
	    counter = 1
	    for year in 1994:2022
	        weeks = if year == 2022
	            40
	        elseif year in [1994, 2000, 2005, 2011, 2016]
	            53
	        else
	            52
	        end
	
	        start_week = if year == 1994
	            6
	        else
	            1
	        end
	
	        for i in start_week:weeks
	            oldvar = Symbol("EMP_STATUS_", year, "_", lpad(i, 2, '0'))
	...
	rename_EMP_STATUS!(NLSY97)
	show(NLSY97)
end

# ╔═╡ 32f7d8ac-c87a-4943-8e4f-ef4d1a07028b
begin
    function drop_columns!(df)
        counter = 1997
        
        while counter <= 2011
            df = select(df, Not(Symbol("INTERVIEWER_PUBID_$counter")))
            counter += 1
        end
    ...
    # Call the function with NLSY97 as an argument and update NLSY97 with the modified DataFrame
    NLSY97_drop = drop_columns!(NLSY97)
...
# ╔═╡ bccf2260-995c-46f1-b0b7-c75ba5a20de7
stacked_NLSY97 = stack(NLSY97_drop, Between(1, 1496), variable_name=:week, value_name=:EMPSTATUS_)


# ╔═╡ 837f7f4d-6a0c-4562-ad4e-6cc573fb4e17
begin
	sort_NLSY97 = sort(stacked_NLSY97, :PUBID_1997)
	show(sort_NLSY97)
end
...
manifest_format = "2.0"
project_hash = "112deb33375388467a9f54cc069873fd1e5d45fd"
