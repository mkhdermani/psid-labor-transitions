...
|---|---|---|
| 1 | `reshape.do`                       | Reshape the PSID extract `mydata.dta` from **wide → long** (one row per person-month); drops unused job-detail variables and reshapes the monthly `WEIGHT_/RELATION_/SEX_/EMP_/UNE_/OUT_` arrays. |
...
- `mydata.dta`, `reshape.dta` — the wide extract and the reshaped long panel.
